 ...
 * @author CC视频
 */
public abstract class MediaPlayActivityBackup<T extends BasePresenter, E extends BaseModel> extends ToolBarActivity<T, E> implements
...
//                Toast.makeText(getApplicationContext(), "已切换至wifi", Toast.LENGTH_SHORT).show();
    ...
    private void showMobileDialog() {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                AlertDialog.Builder builder = new AlertDialog.Builder(MediaPlayActivityBackup.this);
...
//        timerTask.cancel();
        if (videoIds.size() > 0) {
            ...
            player.setDefaultDefinition(defaultDefinition);
            player.setVideoPlayInfo(videoId, ConfigUtil.USERID, ConfigUtil.API_KEY, MediaPlayActivityBackup.this);
...
//			firstInitDefinition = false;
    ...
    private int getCurrentVideoIndex() {
        return Arrays.asList(videoIds).indexOf(videoId);
    }

    private void initPlayChangeVideoPopupWindow() {
        playChangeVideoPopupWindow = new PlayChangeVideoPopupWindow(this, surfaceView.getHeight(),null);
