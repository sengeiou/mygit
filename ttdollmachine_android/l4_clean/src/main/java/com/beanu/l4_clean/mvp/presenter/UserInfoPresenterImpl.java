...
public class UserInfoPresenterImpl extends UserInfoContract.Presenter {
    ...
    @Override
    public void updateUserInfo(String icon, String nickName, String motto, String sex) {
        if (uploadComplete) {
            mView.showProgress();

            mModel.updateUserInfo(icon, nickName, motto, sex).subscribe(new Observer<String>() {
                @Override
                public void onSubscribe(Disposable d) {
                    mRxManage.add(d);
                }
